...
module.exports = function (chart) {
  ...
  var line = function (points, closed, closedY) {
    var path = ''
    ...
    for (var i = 0, length = points.length; i < length; i += 1) {
      if (points[i]) {
        ...
        if (closed) {
          yLo = Math.min(yLo, closedY || 0)
          yHi = Math.max(yHi, closedY || 0)
        }
  ...
  function plotLine (selection) {
    selection.each(function (d) {
      ...
      var index = d.index
      var closed = d.closed
      var closedY = d.closedY
      var closedOpaque = d.closedOpaque
      ...
      innerSelection
        ...
        .attr('opacity', closed && !closedOpaque ? 0.20 : 1)
        .attr('d', function (d) {
          return line(d, closed, closedY)
        })
        .attr(d.attr)
