...
module.exports = function (chart) {
  ...
  function isBetweenRange (range, value) {
  ...
  function checkAutoUpdate (d) {
    ...
    if (d.derivative.updateOnMouseMove && !d.derivative.$$mouseListener) {
      d.derivative.$$mouseListener = function (x0) {
        ...
        // console.log(d.range[0], '-', d.range[1], '->', x0, isBetweenRange(d.range, x0));
        // if(!isBetweenRange(d.range, x0)) {
        //   hideDerivative(d);
        //   return;
        // }
        d.derivative.x0 = x0
        // trigger update (selection = self)
        derivative(self, !isBetweenRange(d.range, x0))
      }
  ...
  derivative = function (selection, hide) {
